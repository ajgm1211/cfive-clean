...
<?php $__env->startSection('title', 'Global Charges LCL'); ?>
...
<script>
    function AbrirModal(action,id){
        ...
        if(action == "editGlobalCharge"){
            var url = '<?php echo e(route("gclcladm.show", ":id")); ?>';
        ...
        if(action == "addGlobalCharge"){
            var url = '<?php echo e(route("gclcladm.add")); ?>';
        ...
        if(action == "duplicateGlobalCharge"){
            var url = '<?php echo e(route("gclcladm.duplicate", ":id")); ?>';
            url = url.replace(':id', id);
            $('.modal-body-add').load(url,function(){
                $('#modalGlobalchargeAdd').modal({show:true});
            });
    ...
    $(function() {
        $('#requesttable').DataTable({
            processing: true,
            //serverSide: true,
            ajax: '<?php echo e(route("gclcladm.create")); ?>',
            columns: [
    ...
    $(document).on('click', '#bulk_delete', function(){
        ...
        swal({
            ...
            if (result.value) {
                ...
                if(id.length > 0)
                {
                    url='<?php echo route("globalchargeslcl.destroyArr",":id"); ?>';
                    url = url.replace(':id', id);
    ...
    $(document).on('click', '#bulk_duplicate', function(){
        ...
        if(id.length > 0){
            url='<?php echo route("gclcladm.duplicate.Array",":id"); ?>';
            url = url.replace(':id', id);
            var token = $("meta[name='csrf-token']").attr("content");
            url = url.replace(':id', id);
