...
<script>
    ...
    function showModal(selector,id){
        if(selector == 1){
            var url = '<?php echo e(route("load.View.Add.country")); ?>';
            $('#modal-body').load(url,function(){
                $('#addCountriesModal').modal();
            });
        } else if(selector == 2){
            var url = '<?php echo e(route("Countries.show",":id")); ?>';
    ...
    $(document).on('click','.BorrarCountry', function(e){
        ...
        swal({
            ...
            if (result.value) {
                ...
                url='<?php echo route("destroy.countrie",":id"); ?>';
                url = url.replace(':id', id);
    ...
    $(function() {
        $('#myatest').DataTable({
            processing: true,
            //serverSide: true,
            ajax: '<?php echo route("Countries.create"); ?>',
            columns: [
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                { data: 'code', name: 'code' },
