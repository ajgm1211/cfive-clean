...
<?php $__env->startSection('title', 'GlobalCharges Adm.'); ?>
...
<script>
    ...
    function AbrirModal(action,id){
        if(action == "editGlobalCharge"){
            var url = '<?php echo e(route("gcadm.show", ":id")); ?>';
        ...
        if(action == "addGlobalCharge"){
            var url = '<?php echo e(route("gcadm.add")); ?>';
            $('#global-body').load(url,function(){
                $('#global-modal').modal({show:true});
            });
        }
        if(action == "duplicateGlobalCharge"){
            var url = '<?php echo e(route("gcadm.dupicate", ":id")); ?>';
            url = url.replace(':id', id);
            $('#global-body').load(url,function(){
                $('#global-modal').modal({show:true});
            });
    ...
    $(function() {
        $('#requesttable').DataTable({
            processing: true,
            //serverSide: true,
            ajax: '<?php echo e(route("gcadm.create")); ?>',
            columns: [
    ...
    $(document).on('click', '#bulk_duplicate', function(){
        ...
        if(id.length > 0)
        {
            url='<?php echo route("gcadm.dupicate.Array",":id"); ?>';
            url = url.replace(':id', id);
            var token = $("meta[name='csrf-token']").attr("content");
            /*$.ajax({
                        url:url,
