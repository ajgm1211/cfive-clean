...
<?php $__env->startSection('title', 'Failed Rates FCL '.$contract['id'].' - '.$contract['number'].' / '.$contract['name']); ?>
                        ...
                        <div class="m-portlet__head">
                            <div class="form-group row ">
                                <div class="col-lg-12">
                                    ...
                                    &nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
                                    <a href="<?php echo e(route('Reprocesar.Rates',$id)); ?>" class="btn btn-primary">Reprocess &nbsp;<span class="la la-refresh"></span></a>
                                </div>
                            </div>
                            <br>
        ...
        <script>
            $(function() {
                $('#myatest').DataTable({
                    processing: true,
                    //serverSide: true,
                    ajax: '<?php echo route("Failed.Rates.Developer.view.For.Contracts",[$id,1]); ?>',
                    columns: [
                ...
                $('#myatest2').DataTable({
                    processing: true,
                    //serverSide: true,
                    ajax: '<?php echo route("Failed.Rates.Developer.view.For.Contracts",[$id,2]); ?>',
                    columns: [
                        { data: 'origin_portLb', name: 'origin_portLb' },
                        { data: 'destiny_portLb', name: 'destiny_portLb' },
                        { data: 'carrierLb', name: 'carrierLb' },
            ...
            function showModalsavetorate(id,operation){
                ...
                if(operation == 1){
                    var url = '<?php echo e(route("Edit.Rates.Fail.For.Contracts", ":id")); ?>';
                    url = url.replace(':id', id);
                    $('#edit-modal-body').load(url,function(){
                        $('#modaleditRate').modal();
                    });
                }else if(operation == 2){
                    var url = '<?php echo e(route("Edit.Rates.Good.For.Contracts", ":id")); ?>';
                    url = url.replace(':id', id);
                    $('#edit-modal-body').load(url,function(){
                        $('#modaleditRate').modal();
                    });
            ...
            $(document).on('click','#delete-FailRate',function(){
                ...
                swal({
                    ...
                    if (result.value) {
                        ...
                        url='<?php echo route("Destroy.RatesF.For.Contracts",":id"); ?>';
                        url = url.replace(':id', id);
            ...
            $(document).on('click','#delete-Rate',function(){
                ...
                swal({
                    ...
                    if (result.value) {
                        ...
                        url='<?php echo route("Destroy.RatesG.For.Contracts",":id"); ?>';
                        url = url.replace(':id', id);
                        // $(this).closest('tr').remove();
                        $.ajax({
                            url:url,
