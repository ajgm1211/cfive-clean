            ...
            @foreach ($freight_charges_detailed as $origin => $value)
                @foreach ($value as $destination => $item)
                    ...
                    <div>
                        ...
                        <p class="title"
                            style="margin-bottom: 0px; color: {{ $user->companyUser->colors_pdf }}">
                            <b>{{ __('pdf.freight_charges') }} - {{ $origin }} | {{ $destination }}</b>
                        </p>
                    ...
                    <table border="0" cellspacing="1" cellpadding="1">
                        ...
                        <thead class="title-quote text-left header-table">
                            ...
                            <tr>
                                ...
                                <th class="unit" {{ @$quote->pdf_options['showCarrier'] ? '' : 'hidden' }}>
                                    <b>{{ __('pdf.carrier') }}</b>
                                </th>
                                    ...
                                    @foreach ($r->charge as $v)
                                        @if ($v->type_id == 3)
                                            ...
                                            <tr class="text-left color-table">
                                                ...
                                                <td>{{ $v->surcharge_id != '' ? $v->surcharge->name : __('pdf.ocean_freight') }}
                                                </td>

                                                <td>{{ $v->surcharge_id != '' ? @$v->calculation_type->name : __('pdf.per_container') }}
                                                </td>

                                                <td {{ @$quote->pdf_options['showCarrier'] ? '' : 'hidden' }}>
                                                    {{ @$r->carrier->name }}</td>
                                                ...
                                                @foreach ($equipmentHides as $key => $hide)
                                                    @foreach ($containers as $c)
                                                        @if ($c->code == $key)
                                                            ...
                                                            <td {{ $hide }}>
                                                                {{ @$total_w_profit == null ? 0 : isDecimal(@$total_w_profit, false, true) . ' ' . $v->currency->alphacode }}
                                                            </td>
                            ...
                            <tr>
                                ...
                                @foreach ($equipmentHides as $key => $hide)
                                    @foreach ($containers as $c)
                                        @if ($c->code == $key)
                                            <td {{ $hide }}>
                                                <b>{{ isDecimal(@$total_freight->${'c' . $c->code}, false, true) . ' ' . @$r->currency->alphacode }}</b>
                                            </td>
                                        @endif
                                    @endforeach
                                @endforeach
