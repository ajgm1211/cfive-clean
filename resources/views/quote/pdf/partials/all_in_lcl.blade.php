@if($quote->pdf_option->show_type=='total in')
    ...
    <div {{$quote->pdf_option->show_type=='total in' ? '':'hidden'}}>
        ...
        <p class="title" style="color: {{ $user->companyUser->colors_pdf }}">{{__('pdf.total_estimated')}}</p>
    ...
    <tbody>
        ...
        @foreach($rates as $rate)
            ...
            <tr class="text-center color-table"> 
                ...
                <td >@if($quote->type=='LCL') {{@$rate->origin_port->name}}, {{@$rate->origin_port->code}} @else {{@$rate->origin_airport->name}}, {{@$rate->origin_airport->code}} @endif</td>

                <td>@if($quote->type=='LCL') {{$rate->destination_port->name}}, {{$rate->destination_port->code}} @else {{$rate->destination_airport->name}}, {{$rate->destination_airport->code}} @endif</td> 
