...
<div class="m-content">
                        ...
                        <table class="table tableData" id="global-table" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr id='tr_l{{++$loop->index}}'>
                                    <td>
                                        <div id="divtype{{$loop->index}}" class="val">{!!
                                            $globalcharges->surcharge->name !!}</div>
                                    </td>
                                    <td>
                                        @if(!$globalcharges->globalcharport->isEmpty())
                                        <div id="divport{{$loop->index}}" class="val">
                                            {!! str_replace(["[","]","\""], ' ',
                                            $globalcharges->globalcharport->pluck('portOrig')->unique()->pluck('display_name')
                                            ) !!}
                                        </div>
                                        @endif
                                        ...
                                        <div id="divcountry{{$loop->index}}" class="val">
                                            {!! str_replace(["[","]","\""], ' ',
                                            $globalcharges->globalcharcountry->pluck('countryOrig')->unique()->pluck('name')
                                            ) !!}
                                        </div>
                                        @endif
                                    </td>
                                    <td>
                                        ...
                                        <div id="divportDest{{$loop->index}}" class="val">
                                            {!! str_replace(["[","]","\""], ' ',
                                            $globalcharges->globalcharport->pluck('portDest')->unique()->pluck('display_name')
                                            ) !!}
                                        </div>
                                        @endif
                                        @if(!$globalcharges->globalcharcountry->isEmpty())
                                        <div id="divcountryDest{{$loop->index}}" class="val">
                                            {!! str_replace(["[","]","\""], ' ',
                                            $globalcharges->globalcharcountry->pluck('countryDest')->unique()->pluck('name')
                                            ) !!}
                                        </div>
                                        @endif
                                    </td>
                                    <td>
                                        <div id="divchangetype{{$loop->index}}" class="val">{!!
                                            $globalcharges->typedestiny->description !!}</div>
                                    </td>
                                    <td>
                                        <div id="divcarrier{{$loop->index}}" class="val">
                                            {!! str_replace(["[","]","\""], ' ',
                                            $globalcharges->GlobalCharCarrier->pluck('carrier')->pluck('name') ) !!}
                                        </div>
                                    </td>
                                    <td>
                                        <div id="divcalculation{{$loop->index}}" class="val">{!!
                                            $globalcharges->calculationtype->name !!}</div>
                                    <td>
                                        <div id="divammount{{$loop->index}}" class="val"> {!! $globalcharges->ammount
                                            !!} </div>
                                    </td>
                                    <td>
                                        <div id="divcurrency{{$loop->index}}" class="val"> {!!
                                            $globalcharges->currency->alphacode !!} </div>
                                    </td>
                                    <td>
                                        <div id="divvalidity{{$loop->index}}" class="val"> {!! $globalcharges->validity
                                            !!} / {!! $globalcharges->expire !!}</div>
                                    </td>
                                    <td>
                                        <a id='edit_l{{$loop->index}}'
                                            onclick="AbrirModal('editGlobalCharge',{{$globalcharges->id}})"
                                            class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"
                                            title="Edit ">
                                            <i class="la la-edit"></i>
                                        </a>
                                        <a id='remove_l{{$loop->index}}'
                                            class="m_sweetalert_demo_8 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"
                                            title="delete">
                                            <i id='rm_l{{$globalcharges->id}}' class="la la-times-circle"></i>
                                        </a>

                                        <a class='m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill test'
                                            title='Duplicate '
                                            onclick='AbrirModal("duplicateGlobalCharge",{{$globalcharges->id}})'>
    ...
    <div class="modal fade bd-example-modal-lg" id="modalGlobalchargeAdd" role="dialog"
