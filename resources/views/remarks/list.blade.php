            ...
            <table class="m-datatable text-center" id="html_table" width="100%">
                ...
                <tbody> 
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($arr->remarksCarriers as $carrier)
                            <ul>
                                <li>{{ $carrier->name }}</li>
                            </ul>
                            @endforeach
                        ...
                        <td>
                            <a href="{{ route('remarks.show', ['id' => setearRouteKey($arr->id)]) }}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill">
                                <i class="la la-eye"></i>
                            </a>
                            <a href="{{ route('remarks.edit', ['id' => setearRouteKey($arr->id)]) }}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"   title="Edit ">
...
<script>
    ...
    function AbrirModal(action,id){
        ...
        if(action == "edit"){
            var url = '{{ route("remarks.edit", ":id") }}';
            url = url.replace(':id', id);
            $('.modal-body').load(url,function(){
                $('#m_modal_5').modal({show:true});
            });
        }if(action == "add"){
            var url = '{{ route("remarks.add") }}';
            $('.modal-body').load(url,function(){
                $('#m_modal_5').modal({show:true});
            });
        }
        if(action == "delete"){
            var url = '{{ route("remarks.msg", "id") }}';
            url = url.replace('id', id);
            $('.modal-body').load(url,function(){
                $('#m_modal_5').modal({show:true});
            });
