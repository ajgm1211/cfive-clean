        ...
        if ($validateEquipment['count'] < 2) {
            if ($companies_id != null || $companies_id != 0) {
                $arreglo = Rate::whereIn('origin_port', $origin_port)->whereIn('destiny_port', $destiny_port)->whereIn('carrier_id', $arregloCarrier)->with('port_origin', 'port_destiny', 'contract')->whereHas('contract', function ($q) use ($dateSince, $dateUntil, $user_id, $company_user_id, $companies_id, $traf) {
                    ...
                    })->where('company_user_id', '=', $company_user_id)->where('status', '!=', 'incomplete')->where('status_erased', '!=', 1)->where('gp_container_id', '=', $validateEquipment['gpId'])->whereIn('direction_id', $traf);
            ...
            if ($contractId != $data->contract->id) {
                ...
                if ($data1 != null) {
                    for ($i = 0; $i < count($data1); $i++) {
                        if(($data1[$i]->deleted_at==null && $data1[$i]->carrier==$data->carrier->name) && (strpos($data1[$i]->port_orig, $data->port_origin->code) && strpos($data1[$i]->port_dest, $data->port_destiny->code) || $data1[$i]->port_orig =="ALL" && strpos($data1[$i]->port_dest, $data->port_destiny->code) || strpos($data1[$i]->port_orig, $data->port_origin->code) && $data1[$i]->port_dest =="ALL" || $data1[$i]->port_orig =="ALL" && $data1[$i]->port_dest =="ALL")){
                                $montosLocal = array();
                            $montosLocal2 = array();
