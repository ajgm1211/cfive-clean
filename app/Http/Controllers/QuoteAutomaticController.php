...
class QuoteAutomaticController extends Controller
{
    ...
    public function inlandDistance($deliveyType, $direccion, $port_id, $currency, $type)
    {
        ...
        if ($deliveyType == '2' || $deliveyType == '4') {
            ...
            $isInland = true;
        } elseif ($deliveyType == '3' || $deliveyType == '4') {
            $harborRate = Harbor::where('id', $port_id)->first();
            $origin = $direccion;
        ...
        ->setParam([
          ...
          'language' => 'es',
            ...
            $arreglo = ['prov_id' => '', 'provider' => 'Inland Haulage', 'providerName' => 'Inland Haulage', 'port_id' => $harborRate->id, 'port_name' =>  $harborRate->name, 'km' => $kilometros, 'monto' => '0.00', 'type' => $type, 'type_currency' => $currency, 'idCurrency' => $currency];

            $arrayDetail = ['cant_cont' => '1', 'sub_in' => '0.00', 'des_in' => 'Inlands', 'amount' => '0.00', 'currency' => 'USD', 'price_unit' => '0'];
            $arraymarkupCero = ['markup' => '0.00', 'markupConvert' => '0.00', 'typemarkup' => $currency];
    ...
    public function listRate(Request $request)
    {
                            ...
                            foreach ($resp->legs as $dist) {
                                ...
                                foreach ($inlandsValue->inlanddetails as $details) {
                                    ...
                                    if ($details->type == 'twuenty' && $request->input('twuenty') != '0') {
                                        if ($distancia >= $details->lower && $distancia <= $details->upper) {
                                            ...
                                            if ($inlandPercentage != 0) {
                                                ...
                                                $monto += $markup;
                                                $arraymarkupT = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($inlandPercentage%)"];
                                            } else {
                                                $markup = $inlandAmmount;
                                                $markup = number_format($markup, 2, '.', '');
                                                $sub_20 += $inlandMarkup;
                                                $monto += $inlandMarkup;
                                                $arraymarkupT = ['markup' => $markup, 'markupConvert' => $inlandMarkup, 'typemarkup' => $markupInlandCurre];
                                            }
                                            // FIN CALCULO MARKUPS

                                            $arrayInland20 = ['cant_cont' => $request->input('twuenty'), 'sub_in' => $sub_20, 'des_in' => $texto20, 'amount' => $amount_inland, 'currency' => $details->currency->alphacode, 'price_unit' => $price_per_unit];
                                    ...
                                    if ($details->type == 'forty' && $request->input('forty') != '0') {
                                        if ($distancia >= $details->lower && $distancia <= $details->upper) {
                                            ...
                                            // FIN CALCULO MARKUPS
                                            $arrayInland40 = ['cant_cont' => $request->input('forty'), 'sub_in' => $sub_40, 'des_in' => $texto40, 'amount' => $amount_inland, 'currency' => $details->currency->alphacode, 'price_unit' => $price_per_unit];
                                    ...
                                    if ($details->type == 'fortyhc' && $request->input('fortyhc') != '0') {
                                        if ($distancia >= $details->lower && $distancia <= $details->upper) {
                                            ...
                                            // FIN CALCULO MARKUPS
                                            $arrayInland40hc = ['cant_cont' => $request->input('fortyhc'), 'sub_in' => $sub_40hc, 'des_in' => $texto40hc, 'amount' => $amount_inland, 'currency' => $details->currency->alphacode, 'price_unit' => $price_per_unit];
                                ...
                                if (isset($inlandsValue->inlandadditionalkms)) {
                                    ...
                                    if ($km20 && $request->input('twuenty') != '0') {
                                        ...
                                        $arrayInland20 = ['cant_cont' => $request->input('twuenty'), 'sub_in' => $sub_20, 'des_in' => $texto20, 'amount' => $amount_inland, 'currency' =>$inlandsValue->inlandadditionalkms->currency->alphacode, 'price_unit' => $price_per_unit];
                                        $arrayInland20 = array_merge($arraymarkupT, $arrayInland20);
                                        $inlandDetails[] = $arrayInland20;
                                    }
                                    if ($km40 && $request->input('forty') != '0') {
                                        ...
                                        $arrayInland40 = ['cant_cont' => $request->input('forty'), 'sub_in' => $sub_40, 'des_in' =>  $texto40, 'amount' => $amount_inland, 'currency' => $inlandsValue->inlandadditionalkms->currency->alphacode, 'price_unit' => $price_per_unit];
                                        $arrayInland40 = array_merge($arraymarkupT, $arrayInland40);
                                        $inlandDetails[] = $arrayInland40;
                                    }
                                    if ($km40hc && $request->input('fortyhc') != '0') {
                                        ...
                                        $arrayInland40hc = ['cant_cont' => $request->input('fortyhc'), 'sub_in' => $sub_40hc, 'des_in' => $texto40hc, 'amount' => $amount_inland, 'currency' => $typeCurrency, 'price_unit' => $price_per_unit];
                                        $arrayInland40hc = array_merge($arraymarkupT, $arrayInland40hc);
                                        $inlandDetails[] = $arrayInland40hc;
                                    }
                                }
                                ...
                                if ($monto > 0) {
                                    ...
                                    $arregloInland = ['prov_id' => $inlandsValue->id, 'provider' => 'Inland Haulage', 'providerName' => $inlandsValue->provider, 'port_id' => $ports->ports->id, 'port_name' =>  $ports->ports->name, 'km' => $distancia, 'monto' => $monto, 'type' => 'Destiny Port To Door', 'type_currency' => $inlandsValue->inlandadditionalkms->currency->alphacode, 'idCurrency' => $inlandsValue->currency_id];
                            ...
                            foreach ($resp->legs as $dist) {
                                ...
                                if (isset($inlandsValue->inlandadditionalkms)) {
                                    ...
                                    if ($km40hc && $request->input('fortyhc') != '0') {
                                        ...
                                        // FIN CALCULO MARKUPS
                                        $sub_40hc = number_format($sub_40hc, 2, '.', '');
                                        $arrayInland40hc = ['cant_cont' => $request->input('fortyhc'), 'sub_in' => $sub_40hc, 'des_in' => $texto40hc, 'amount' => $amount_inland, 'currency' => $inlandsValue->inlandadditionalkms->currency->alphacode, 'price_unit' => $price_per_unit];
                                        $arrayInland40hc = array_merge($arraymarkupT, $arrayInland40hc);
                                        $inlandDetailsOrig[] = $arrayInland40hc;
                                    }
                                }
                                ...
                                if ($monto > 0) {
                                    ...
                                    $arregloInland = ['prov_id' => $inlandsValue->id, 'provider' => 'Inland Haulage', 'providerName' => $inlandsValue->provider, 'port_id' => $ports->ports->id, 'port_name' =>  $ports->ports->name, 'km' => $distancia, 'monto' => $monto, 'type' => 'Origin Port To Door', 'type_currency' => $typeCurrency, 'idCurrency' => $inlandsValue->currency_id];
      ...
      if (! empty($dataOrig)) {
        ...
        $formulario = $request;
        $array20 = ['2', '4', '5']; // id  calculation type 2 = per 20 , 4= per teu , 5 per container
    $array40 = ['1', '4', '5']; // id  calculation type 2 = per 40
    $array40Hc = ['3', '4', '5']; // id  calculation type 3 = per 40HC
    $array40Nor = ['7', '4', '5'];  // id  calculation type 7 = per 40NOR
    $array45 = ['8', '4', '5'];  // id  calculation type 8 = per 45
    $collectionLocal = new Collection();
        foreach ($arreglo as $data) {
            ...
            if ($request->input('twuenty') != '0') {
                ...
                $totalFreight += $totalT;
                $totalRates += $totalT;
                $array = ['type'=>'Ocean Freight 20', 'cantidad' => $formulario->twuenty, 'detail'=>'Container 20', 'price' => $data->twuenty, 'currency' => $data->currency->alphacode, 'subtotal' => $subtotalT, 'total' =>$totalT.' '.$typeCurrency, 'idCurrency' => $data->currency_id];
            ...
            if ($request->input('forty') != '0') {
                ...
                if ($freighPercentage != 0) {
                    ...
                    $markup = number_format($markup, 2, '.', '');
                    $totalF += $markup;
                    $arraymarkupF = ['markup' => $markup, 'markupConvert' => $markup,  'typemarkup' => "$typeCurrency ($freighPercentage%)"];
                } else {
                    $markup = trim($freighAmmount);
                    $markup = number_format($markup, 2, '.', '');
                    $totalF += $freighMarkup;
                    $arraymarkupF = ['markup' => $markup, 'markupConvert' => $freighMarkup, 'typemarkup' => $markupFreightCurre];
                }
                $totalF = number_format($totalF, 2, '.', '');
                $totalFreight += $totalF;
                $totalRates += $totalF;
                $array = ['type'=>'Ocean Freight 40', 'cantidad' => $formulario->forty, 'detail'=>'Container 40', 'price' => $data->forty, 'currency' => $data->currency->alphacode, 'subtotal' => $subtotalF, 'total' =>$totalF.' '.$typeCurrency, 'idCurrency' => $data->currency_id];
            ...
            if ($request->input('fortyhc') != '0') {
                ...
                if ($freighPercentage != 0) {
                    ...
                    $markup = number_format($markup, 2, '.', '');
                    $totalFHC += $markup;
                    $arraymarkupFH = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($freighPercentage%)"];
                } else {
                    $markup = trim($freighAmmount);
                    $markup = number_format($markup, 2, '.', '');
                    $totalFHC += $freighMarkup;
                    $arraymarkupFH = ['markup' => $markup, 'markupConvert' => $freighMarkup, 'typemarkup' => $markupFreightCurre];
                }
                $totalFHC = number_format($totalFHC, 2, '.', '');
                $totalFreight += $totalFHC;
                $totalRates += $totalFHC;
                $array = ['type'=>'Ocean Freight 40HC', 'cantidad' => $formulario->fortyhc, 'detail'=>'Container 40HC', 'price' => $data->fortyhc, 'currency' => $data->currency->alphacode, 'subtotal' => $subtotalFHC, 'total' =>$totalFHC.' '.$typeCurrency, 'idCurrency' => $data->currency_id];
            ...
            if ($request->input('fortynor') != '0') {
                ...
                if ($freighPercentage != 0) {
                    ...
                    $markup = number_format($markup, 2, '.', '');
                    $totalNOR += $markup;
                    $arraymarkupNOR = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($freighPercentage%)"];
                } else {
                    $markup = trim($freighAmmount);
                    $markup = number_format($markup, 2, '.', '');
                    $totalNOR += $freighMarkup;
                    $arraymarkupNOR = ['markup' => $markup, 'markupConvert' => $freighMarkup, 'typemarkup' => $markupFreightCurre];
                }
                $totalNOR = number_format($totalNOR, 2, '.', '');
                $totalFreight += $totalNOR;
                $totalRates += $totalNOR;
                $array = ['type'=>'Ocean Freight 40NOR ', 'cantidad' => $formulario->fortynor, 'detail'=>'Container 40NOR', 'price' => $data->fortynor, 'currency' => $data->currency->alphacode, 'subtotal' => $subtotalNOR, 'total' =>$totalNOR.' '.$typeCurrency, 'idCurrency' => $data->currency_id];
            ...
            if ($request->input('fortyfive') != '0') {
                ...
                if ($freighPercentage != 0) {
                    ...
                    $markup = number_format($markup, 2, '.', '');
                    $totalFIVE += $markup;
                    $arraymarkupFIVE = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($freighPercentage%)"];
                } else {
                    $markup = trim($freighAmmount);
                    $markup = number_format($markup, 2, '.', '');
                    $totalFIVE += $freighMarkup;
                    $arraymarkupFIVE = ['markup' => $markup, 'markupConvert' => $freighMarkup, 'typemarkup' => $markupFreightCurre];
                }
                $totalFIVE = number_format($totalFIVE, 2, '.', '');
                $totalFreight += $totalFIVE;
                $totalRates += $totalFIVE;
                $array = ['type'=>'Ocean Freight 45', 'cantidad' => $formulario->fortyfive, 'detail'=>'Container 45', 'price' => $data->fortyfive, 'currency' => $data->currency->alphacode, 'subtotal' => $subtotalFIVE, 'total' =>$totalFIVE.' '.$typeCurrency, 'idCurrency' => $data->currency_id];
                            ...
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($local->typedestiny_id == '1') {
                                    ...
                                    if ($localPercentage != 0) {
                                        ...
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $markup;
                                        $arraymarkupT = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                    } else {
                                        $markup = $localAmmount;
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $localMarkup;
                                        $arraymarkupT = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                    }
                                    $totalOrigin += $totalAmmount;
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $formulario->twuenty, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '2') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $formulario->twuenty, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '3') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $formulario->twuenty, 'idCurrency' => $local->currency->id];
                        ...
                        foreach ($local->localcharcarriers as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($local->typedestiny_id == '1') {
                                    if ($local->calculationtype_id == '4') {
                                        ...
                                        if ($localPercentage != 0) {
                                            ...
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $markup;
                                            $arraymarkupF = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                        } else {
                                            $markup = $localAmmount;
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $localMarkup;
                                            $arraymarkupF = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                        }
                                        $totalOrigin += $totalAmmount;
                                    } else {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '2') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '3') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                        ...
                        foreach ($local->localcharcarriers as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($local->typedestiny_id == '1') {
                                    if ($local->calculationtype_id == '4') {
                                        ...
                                        if ($localPercentage != 0) {
                                            ...
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $markup;
                                            $arraymarkupFH = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                        } else {
                                            $markup = $localAmmount;
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $localMarkup;
                                            $arraymarkupFH = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                        }
                                        $totalOrigin += $totalAmmount;
                                    } else {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '2') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '3') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                        ...
                        foreach ($local->localcharcarriers as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($local->typedestiny_id == '1') {
                                    if ($local->calculationtype_id == '4') {
                                        ...
                                        if ($localPercentage != 0) {
                                            ...
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $markup;
                                            $arraymarkupNor = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                        } else {
                                            $markup = $localAmmount;
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $localMarkup;
                                            $arraymarkupNor = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                        }
                                        $totalOrigin += $totalAmmount;
                                    } else {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '2') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '3') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                        ...
                        foreach ($local->localcharcarriers as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($local->typedestiny_id == '1') {
                                    if ($local->calculationtype_id == '4') {
                                        ...
                                        if ($localPercentage != 0) {
                                            ...
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $markup;
                                            $arraymarkupFortyFive = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                        } else {
                                            $markup = $localAmmount;
                                            $markup = number_format($markup, 2, '.', '');
                                            $totalAmmount += $localMarkup;
                                            $arraymarkupFortyFive = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                        }
                                        $totalOrigin += $totalAmmount;
                                    } else {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '2') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                                ...
                                if ($local->typedestiny_id == '3') {
                                    ...
                                    $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\'  Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                        ...
                        if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                            if ($local->typedestiny_id == '1') {
                                ...
                                if ($localPercentage != 0) {
                                    ...
                                    $markup = number_format($markup, 2, '.', '');
                                    $totalAmmount += $markup;
                                    $arraymarkupPC = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                } else {
                                    $markup = $localAmmount;
                                    $markup = number_format($markup, 2, '.', '');
                                    $totalAmmount += $localMarkup;
                                    $arraymarkupPC = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                }
                                $totalOrigin += $totalAmmount;
                                $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => '-', 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                            ...
                            if ($local->typedestiny_id == '2') {
                                ...
                                $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => '-', 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                            ...
                            if ($local->typedestiny_id == '3') {
                                ...
                                $totalFreight += $totalAmmount;
                                $FreightCharges += $totalAmmount;
                                $arregloPC = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => '-', 'monto' => $local->ammount, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtype->name, 'contract_id' => $data->contract_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                        ...
                        foreach ($global->globalcharcarrier as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($global->typedestiny_id == '1') {
                                    ...
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloOrig = array_merge($arregloOrig, $arraymarkupT);
                                ...
                                if ($global->typedestiny_id == '2') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloDest = array_merge($arregloDest, $arraymarkupT);
                                ...
                                if ($global->typedestiny_id == '3') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->twuenty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'20\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloFreight = array_merge($arregloFreight, $arraymarkupT);
                        ...
                        foreach ($global->globalcharcarrier as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($global->typedestiny_id == '1') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloOrig = array_merge($arregloOrig, $arraymarkupF);
                                ...
                                if ($global->typedestiny_id == '2') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloDest = array_merge($arregloDest, $arraymarkupF);
                                ...
                                if ($global->typedestiny_id == '3') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->forty, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloFreight = array_merge($arregloFreight, $arraymarkupF);
                        ...
                        foreach ($global->globalcharcarrier as $carrierGlobal) {
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($global->typedestiny_id == '1') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloOrig = array_merge($arregloOrig, $arraymarkupFH);
                                ...
                                if ($global->typedestiny_id == '2') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloDest = array_merge($arregloDest, $arraymarkupFH);
                                ...
                                if ($global->typedestiny_id == '3') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyhc, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' HC Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                    $arregloFreight = array_merge($arregloFreight, $arraymarkupFH);
                            ...
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($global->typedestiny_id == '1') {
                                    ...
                                    if ($localPercentage != 0) {
                                        ...
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $markup;
                                        $arraymarkupGNor = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                    } else {
                                        $markup = $localAmmount;
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $localMarkup;
                                        $arraymarkupGNor = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                    }
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' NOR Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                ...
                                if ($global->typedestiny_id == '2') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' NOR Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                ...
                                if ($global->typedestiny_id == '3') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortynor, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'40\' NOR Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                            ...
                            if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                                if ($global->typedestiny_id == '1') {
                                    ...
                                    if ($localPercentage != 0) {
                                        ...
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $markup;
                                        $arraymarkupFortyfiveG = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                    } else {
                                        $markup = $localAmmount;
                                        $markup = number_format($markup, 2, '.', '');
                                        $totalAmmount += $localMarkup;
                                        $arraymarkupFortyfiveG = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                    }
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'45\'  ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                ...
                                if ($global->typedestiny_id == '2') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'45\'  ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                ...
                                if ($global->typedestiny_id == '3') {
                                    ...
                                    $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                    $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                    $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $formulario->fortyfive, 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'45\'  ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                    ...
                    foreach ($global->globalcharcarrier as $carrierGlobal) {
                        if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                            if ($global->typedestiny_id == '1') {
                                ...
                                $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => '-', 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=> 'Shipment Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                $arregloOrig = array_merge($arregloOrig, $arraymarkupPC);
                            ...
                            if ($global->typedestiny_id == '2') {
                                ...
                                $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => '-', 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=> 'Shipment Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                $arregloDest = array_merge($arregloDest, $arraymarkupPC);
                            ...
                            if ($global->typedestiny_id == '3') {
                                ...
                                $totalFreight += $totalAmmount;
                                $FreightCharges += $totalAmmount;
                                $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => '-', 'monto' => $global->ammount, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtype->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=> 'Shipment Global ', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
