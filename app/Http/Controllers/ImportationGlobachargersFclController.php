...
class ImportationGlobachargersFclController extends Controller
{
    ...
    public function changeEquiment(Request $request,$id){
        $account            = AccountImportationGlobalcharge::with('companyuser')->find($id);
        $groupContainers    = GroupContainer::all();
        $equiment           = $groupContainers->pluck('name','id');
        $request_arr  = [
            'id'        => 'Manual',
            'date'      => null,
            'equiement' => 'Dry',
        ...
        if(empty($account->requestgc_id) != true){
            $request = NewGlobalchargeRequestFcl::find($account->requestgc_id);
            if(json_decode($request->data,true) != null){
                $color = '#012586';
                $data   = json_decode($request->data,true);
                if(!empty($data['group_containers'])){
                    $request_arr['equiement_id']    = $data['group_containers']['id'];
            ...
            $request_arr['id']          = $request->id;
            $request_arr['date']        = $request->created;
            $request_arr['equiement']   = $name;
            $request_arr['color']       = $color;
        } 
        $data = [
            'id'            => $account->id,
            'name'          => $account->name,
            'date'          => $account->date,
            'companyuser'   => $account->companyuser->name,
            'status'        => $account->status,
            'equiment'      => $equiment,
            'request'       => $request_arr,
        ];

        return view('RequestGlobalChargeFcl.Body-Modals.changeEquiment',compact('data','equiment','account'));
    }
