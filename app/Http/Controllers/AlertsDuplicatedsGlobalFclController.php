        ...
        return DataTables::of($alerts)
            ->addColumn('status', function ($alerts) {
                ...
                if (strnatcasecmp($alerts->status->name, 'pending') == 0) {
                    //$color = 'color:#031B4E';
                    $color = 'color:#f81538';
                } elseif (strnatcasecmp($alerts->status->name, 'false') == 0) {
                    $color = 'color:#5527f0';
                } elseif (strnatcasecmp($alerts->status->name, 'solved') == 0) {
                    $color = 'color:#e07000';
                } else {
                    $color = 'color:#04950f';
                }
    ...
    public function updateStatus(Request $request, $id)
    {
        try {
            ...
            if (strnatcasecmp($alert->status->name, 'pending') == 0) {
                $color = '#f81538';
            } elseif (strnatcasecmp($alert->status->name, 'false') == 0) {
                $color = '#5527f0';
            } elseif (strnatcasecmp($alert->status->name, 'solved') == 0) {
                $color = '#e07000';
            } else {
                $color = '#04950f';
            }
