...
class QuoteAutomaticLclController extends Controller
{
    ...
    // Variables de configuracion
        ...
        foreach ($arreglo as $data) {
            ...
            $arrayBlHblShip = ['1', '2', '3']; // id  calculation type 1 = HBL , 2=  Shipment , 3 = BL
      $arraytonM3 = ['4']; //  calculation type 4 = Per ton/m3
      $arraytonCompli = ['6', '7']; //  calculation type 4 = Per ton/m3
      $arrayPerTon = ['5']; //  calculation type 5 = Per  TON
                        ...
                        if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                            if ($local->typedestiny_id == '1') {
                                ...
                                if ($localPercentage != 0) {
                                    ...
                                    $markup = number_format($markup, 2, '.', '');
                                    $totalAmmount += $markup;
                                    $arraymarkupTon = ['markup' => $markup, 'markupConvert' => $markup, 'typemarkup' => "$typeCurrency ($localPercentage%)"];
                                } else {
                                    $markup = $localAmmount;
                                    $markup = number_format($markup, 2, '.', '');
                                    $totalAmmount += $localMarkup;
                                    $arraymarkupTon = ['markup' => $markup, 'markupConvert' => $localMarkup, 'typemarkup' => $markupLocalCurre];
                                }
                                $totalOrigin += $totalAmmount;
                                $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $weight, 'monto' => $mont, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtypelcl->name, 'contract_id' => $data->contractlcl_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                            ...
                            if ($local->typedestiny_id == '2') {
                                ...
                                $subtotal_local = number_format($subtotal_local, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $weight, 'monto' => $mont, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtypelcl->name, 'contract_id' => $data->contractlcl_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                            ...
                            if ($local->typedestiny_id == '3') {
                                ...
                                $totalFreight += $totalAmmount;
                                $FreightCharges += $totalAmmount;
                                $arregloPC = ['surcharge_terms' => $terminos, 'surcharge_name' => $local->surcharge->name, 'cantidad' => $weight, 'monto' => $mont, 'currency' => $local->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $local->calculationtypelcl->name, 'contract_id' => $data->contractlcl_id, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>' Shipment Local ', 'subtotal_local' => $subtotal_local, 'cantidadT' => $cantidadT, 'idCurrency' => $local->currency->id];
                    ...
                    foreach ($global->globalcharcarrierslcl as $carrierGlobal) {
                        if ($carrierGlobal->carrier_id == $data->carrier_id || $carrierGlobal->carrier_id == $carrier_all) {
                            if ($global->typedestiny_id == '1') {
                                ...
                                $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloOrig = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $cantidadT, 'monto' => $mont, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtypelcl->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'Ship/Hbl/Bl', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                $arregloOrig = array_merge($arregloOrig, $arraymarkupT);
                                $origGlo['origin'] = $arregloOrig;
                                $collectionGloOrig->push($origGlo);
                            }
                            if ($global->typedestiny_id == '2') {
                                ...
                                $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloDest = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $cantidadT, 'monto' => $mont, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtypelcl->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'Ship/Hbl/Bl', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                $arregloDest = array_merge($arregloDest, $arraymarkupT);
                                $destGlo['destiny'] = $arregloDest;
                                $collectionGloDest->push($destGlo);
                            }
                            if ($global->typedestiny_id == '3') {
                                ...
                                $subtotal_global = number_format($subtotal_global, 2, '.', '');
                                $totalAmmount = number_format($totalAmmount, 2, '.', '');
                                $arregloFreight = ['surcharge_terms' => $terminos, 'surcharge_name' => $global->surcharge->name, 'cantidad' => $cantidadT, 'monto' => $mont, 'currency' => $global->currency->alphacode, 'totalAmmount' =>  $totalAmmount.' '.$typeCurrency, 'calculation_name' => $global->calculationtypelcl->name, 'carrier_id' => $carrierGlobal->carrier_id, 'type'=>'Ship/Hbl/Bl', 'subtotal_global' => $subtotal_global, 'cantidadT' => $cantidadT, 'idCurrency' => $global->currency->id];
                                $arregloFreight = array_merge($arregloFreight, $arraymarkupT);
                                $freighGlo['freight'] = $arregloFreight;
                                $collectionGloFreight->push($freighGlo);
                            }
