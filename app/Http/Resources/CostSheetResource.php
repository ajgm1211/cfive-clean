    ...
    {   
        ...
        foreach ($inlands as $inland) {
            ...
            if ($inland->port_id == $originRate || $inland->port_id == $destinoRate) {
                ...
                array_push($buyingAmountAll, $this->getAmountPerContainer($ratesInland));

                // Valor original(rate) + profit(markup) de todos los inland asociados a los puertos del flete 
                foreach ($ratesInland as $keyRate => $valRate) {                          
                    foreach ($markupInland as $keyMarkup => $valMarkup) { 
        ...
        // Obtener charges del rate, de su origen y su destino
        foreach ($local_charge_quotes_model as $local) {
            if ($local->port_id == $originRate || $local->port_id == $destinoRate) {
                ...
                // acumular para calcular subtotal de compra
                array_push($buyingAmountAll, $this->getAmountPerContainer($local['price']));

                // acumular para calcular subtotal de venta
                array_push($sellingAmountAll, $this->getAmountPerContainer($local['total']));
        ...
        foreach ($freightChargesModel as $charge) {
            ...
            array_push($freightCharges, [
                ...
                'amount' => $this->getAmountPerContainer($freightAmountsCharges)
            ]);              
            
            // acumular para calcular subtotal de compra
            array_push($buyingAmountAll, $this->getAmountPerContainer($freightAmountsCharges));              
        }
