...
class Contract extends Model implements HasMedia, Auditable
{
    ...
    protected $fillable = ['id', 'name', 'number', 'company_user_id', 'account_id', 'direction_id', 'validity', 'expire', 'status', 'remarks', 'gp_container_id', 'code','is_manual'];
    ...
    public function processSearchByIdFcl($response = false, $convert = false)
    {
        ...
        if ($validateEquipment['count'] < 2) {
            $data = Rate::whereHas('contract', function ($q) use ($company_user_id) {
